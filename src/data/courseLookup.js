...
import { supplementaryCourseAliases, supplementaryCourseDetails } from './supplementaryCourseDetails'
...
export function getAdjacentUnlockedActivities(courseId, activityId) {
  const activities = getCourseActivityContexts(courseId)
    .filter((item) => item.activity.status !== 'locked' && getActivityRouteType(item.activity))
  ...
  return {
    ...
    next: currentIndex >= 0 && currentIndex < activities.length - 1 ? activities[currentIndex + 1] : null,
  }
}
