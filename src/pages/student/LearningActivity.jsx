...
function VideoScreen({ activity, onAction }) {
  return (
    <>
      <ActivityHeader activity={activity} label={activity.type === 'Buổi giải đề' ? 'Buổi giải đề' : 'Video'} />
...
function getBestAttempt(attempts, fallback) {
  return attempts.reduce((best, item) => (
...
function ResultSummary({ activity, label, questions, attempt, onReview, onRetry, onBack, history = [], onOpenAttempt }) {
    ...
    <>
      <article className={`hl-activity-card hl-activity-result-card is-${scoreTone}`}>
        <div className="hl-activity-result-main">
          <span>{label}</span>
          <h1>{label} · {activity.title}</h1>
          {meta.length > 0 && <p>{meta.join(' · ')}</p>}
          <div className="hl-activity-result-meta-line">
            <span>Lần làm <b className="is-attempt">{attempt.attemptNumber}</b></span>
            <i />
            <span>Thời gian <b className="is-duration">{attempt.duration}</b></span>
            <i />
            <span>Ngày nộp <b className="is-date">{formatShortDate(attempt.submittedAt)}</b></span>
          </div>
          <div className="hl-activity-result-actions">
            <button type="button" className="is-primary" onClick={() => onReview(attempt)}>Xem lại bài làm</button>
            <button type="button" className="is-secondary" onClick={onRetry}><RotateCcw size={15} />Làm lại</button>
          </div>
        </div>

        <div className="hl-activity-result-score">
          <div className="hl-activity-result-bubble">Điểm được cập nhật<br />theo kết quả cao nhất</div>
          <img src="/owl-mascot4.png" alt="" aria-hidden="true" />
          <div className="hl-activity-result-score-panel">
            <span>Điểm cao nhất</span>
            <h2><b>{bestAttempt.score}</b> <small>/ {bestAttempt.totalQuestions}</small></h2>
          </div>
        </div>
      </article>
          ...
          <div>
            {history.map((item) => (
              <article key={item.id} className="hl-activity-attempt-row">
                ...
                <button type="button" onClick={() => onOpenAttempt(item)}>Xem lại</button>
              </article>
            ))}
          </div>
        </section>
...
function ReviewQuestion({ question, attempt }) {
  ...
  const state = !selectedAnswer ? 'unanswered' : selectedAnswer === question.correctAnswer ? 'correct' : 'incorrect'
  const stateLabel = state === 'correct' ? 'Đúng' : state === 'incorrect' ? 'Sai' : 'Chưa trả lời'

  return (
    <article id={`review-question-${question.number}`} className={`hl-activity-card hl-activity-question-item hl-activity-review-question is-${state}`}>
      ...
      <div className="hl-activity-review-options">
        {question.options.map((option) => {
          ...
          return (
            <p key={option.id} className={`${isSelected ? 'is-student' : ''} ${isCorrect ? 'is-correct' : ''}`}>
              <b>{option.id}</b>
      ...
      <div className="hl-activity-review-answer">
        <p><span>Đáp án của bạn:</span><strong>{selectedOption ? `${selectedOption.id}. ${selectedOption.text}` : 'Chưa trả lời'}</strong></p>
        <p><span>Đáp án đúng:</span><strong>{correctOption.id}. {correctOption.text}</strong></p>
      </div>
...
function ReviewMode({ activity, label, questions, attempt, onBackResult }) {
  ...
  const totalPages = Math.max(1, Math.ceil(questions.filter((question) => {
    const answer = attempt.answers[question.id]
    const state = !answer ? 'unanswered' : answer === question.correctAnswer ? 'correct' : 'incorrect'
  ...
  const jumpToQuestion = (number) => {
    ...
    window.setTimeout(() => {
      document.getElementById(`review-question-${number}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }, 80)
  }
    ...
    <>
      ...
      <div className="hl-activity-review-filters">
        {reviewFilters.map((item) => (
          <button key={item.key} type="button" className={filter === item.key ? 'is-active' : ''} onClick={() => setReviewFilter(item.key)}>
            {item.label}
          </button>
        ))}
      </div>
      <div className="hl-activity-test-layout">
        <main className="hl-activity-question-stack">
          {visibleQuestions.map((question) => <ReviewQuestion key={question.id} question={question} attempt={attempt} />)}
          {visibleQuestions.length === 0 && (
            <article className="hl-activity-card hl-activity-review-empty">Không có câu hỏi phù hợp với bộ lọc này.</article>
          )}
        ...
        <aside className="hl-activity-card hl-activity-question-nav">
          ...
          <div className="hl-activity-tracker-stat">
            <p><span>Điểm</span><strong>{attempt.score}/{attempt.totalQuestions}</strong></p>
            <p><span>Tỷ lệ</span><strong>{attempt.percentage}%</strong></p>
            <p><span>Sai</span><strong>{breakdown.incorrect}</strong></p>
            <p><span>Chưa trả lời</span><strong>{breakdown.unanswered}</strong></p>
          </div>
          <div className="hl-activity-question-number-grid">
            {questions.map((question) => {
              ...
              const state = !answer ? 'unanswered' : answer === question.correctAnswer ? 'correct' : 'incorrect'
              return (
                <button key={question.id} type="button" className={`is-review-${state}`} onClick={() => jumpToQuestion(question.number)}>
                  {question.number}
                </button>
              )
            })}
          ...
          <div className="hl-activity-tracker-legend">
            <span><i className="is-correct" /> Đúng</span>
            <span><i className="is-incorrect" /> Sai</span>
            <span><i className="is-unanswered" /> Chưa trả lời</span>
          </div>
          <button type="button" className="hl-activity-submit-button" onClick={onBackResult}>Quay lại kết quả</button>
        </aside>
      </div>
    </>
  )
...
function QuestionTakingScreen({ activity, mode, onBack }) {
  const totalQuestions = activity.questionCount || (mode === 'mock-test' ? 120 : mode === 'mini-test' ? 20 : 10)
  const questions = useMemo(() => createQuestions(activity, totalQuestions), [activity, totalQuestions])
  const label = mode === 'mock-test' ? 'Mock Test' : mode === 'mini-test' ? 'Mini Test' : 'Bài tập'
  const [attempts, setAttempts] = useState(() => getStoredAttempts(activity.id, createSeedAttempt(activity, questions)))
  ...
  const durationSeconds = parseDurationToSeconds(activity.duration, mode === 'mock-test' ? 150 : mode === 'mini-test' ? 25 : 30)
  const timerText = mode === 'exercise'
    ...
    <>
      ...
      <div className="hl-activity-test-layout">
        ...
        <aside className="hl-activity-card hl-activity-question-nav">
          ...
          <div className="hl-activity-tracker-stat">
            <p><span>{mode === 'exercise' ? 'Thời gian' : 'Còn lại'}</span><strong>{timerText}</strong></p>
            <p><span>Đã làm</span><strong>{answeredCount}/{totalQuestions}</strong></p>
          </div>
          <div className="hl-activity-question-number-grid">
            {questions.map((question) => (
              <button
          ...
          <div className="hl-activity-tracker-legend">
            ...
            <span><i /> Chưa trả lời</span>
          </div>
          <button type="button" className="hl-activity-submit-button" onClick={submit}>NỘP BÀI</button>
        </aside>
      </div>
      ...
      {confirmOpen && (
        <div className="hl-activity-modal-layer">
          <div className="hl-activity-submit-modal">
            ...
            <div>
              <button type="button" onClick={() => setConfirmOpen(false)}>Tiếp tục làm</button>
              <button type="button" onClick={finishAttempt}>Nộp bài</button>
            </div>
          </div>
        </div>
      )}
...
function LearningActions({ primaryLabel, onAction }) {
  return (
    <div className="hl-activity-actions">
      <button type="button" onClick={() => onAction('Điều hướng bài trước đang được phát triển.')}><ArrowLeft size={15} />Bài trước</button>
      <button type="button" className="is-primary" onClick={() => onAction('Tiến độ đã được cập nhật mô phỏng.')}><CheckCircle2 size={15} />{primaryLabel}</button>
      <button type="button" onClick={() => onAction('Điều hướng bài tiếp theo đang được phát triển.')}>Bài tiếp theo<ArrowRight size={15} /></button>
    </div>
  )
}
...
function LearningActivity({ courseId, routeType, activityId, onBack }) {
  ...
  return (
    <LearningShell context={context} onBack={onBack}>
      ...
      {routeType === 'exercises' && <QuestionTakingScreen activity={activity} mode="exercise" onBack={onBack} />}
      {routeType === 'mini-tests' && <QuestionTakingScreen activity={activity} mode="mini-test" onBack={onBack} />}
      {routeType === 'mock-tests' && <QuestionTakingScreen activity={activity} mode="mock-test" onBack={onBack} />}
