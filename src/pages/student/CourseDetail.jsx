...
function CurriculumSection({ section, index, openState, toggle, onAction, onOpenActivity }) {
  ...
  return (
    <article className="hl-course-detail-accordion">
      <button type="button" className="hl-course-detail-accordion-head" aria-expanded={sectionOpen} onClick={() => toggle('sections', section.id)}>
        <div>
          <span>
            <ChevronDown size={17} />
          ...
          {section.groups.map((group) => {
            ...
            return (
              <article key={group.id} className="hl-course-detail-group">
                <button type="button" className="hl-course-detail-group-head" aria-expanded={groupOpen} onClick={() => toggle('groups', group.id)}>
                  <div>
                    <span><ChevronDown size={16} />{group.title}</span>
  ...
  return (
    <article className="hl-course-detail-accordion hl-course-detail-full-mock">
      <button type="button" className="hl-course-detail-accordion-head" aria-expanded={open} onClick={onToggle}>
        ...
        <strong>{course.fullMockTests.filter((item) => item.status === 'completed').length}/{course.fullMockTests.length}</strong>
      </button>
      {open && (
        <div className="hl-course-detail-activity-list">
  ...
  return (
    <>
      {course.chapters.map((chapter, index) => (
        <article key={chapter.id} className="hl-course-detail-accordion hl-course-detail-supp-chapter">
          <ChapterAccordion
            chapter={chapter}
            index={index}
            open={openState.chapters.includes(chapter.id)}
      ...
      <article className="hl-course-detail-accordion hl-course-detail-full-mock">
        <button type="button" className="hl-course-detail-accordion-head" aria-expanded={openState.finalTest} onClick={() => toggle('finalTest')}>
          <div>
            <span>
              <ChevronDown size={17} />
    ...
    <section className="hl-student-page hl-course-detail-page">
        ...
        <div className="hl-course-detail-hero-grid">
          <div>
            ...
            <div className="hl-course-detail-meta">
              <span><GraduationCap size={16} />Giảng viên: {course.instructor}</span>
              <span><BookOpen size={16} />{course.totalContent || course.totalLessons} nội dung học</span>
            </div>
          </div>
          <div className="hl-course-detail-hero-action">
            <span>Tiến độ khóa học</span>
            <div>
              <small>{course.completedContent || course.completedLessons}/{course.totalContent || course.totalLessons} bài học</small>
              <strong>{course.progress}%</strong>
            </div>
            <ProgressLine value={course.progress} />
            <button type="button" onClick={() => showMessage('Trình phát video đang được phát triển.')}>
              Tiếp tục học
              <Play size={15} />
            </button>
          </div>
      ...
      <main className="hl-course-detail-curriculum">
        <div className="hl-course-detail-content-head">
          ...
          <p>Học tuần tự từ phần đang mở, hoàn thành từng hoạt động để mở khóa các bài tiếp theo.</p>
        </div>
