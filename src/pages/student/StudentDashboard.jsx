  ...
  return (
    <div className="hl-dashboard-level">
      ...
      <div className="hl-dashboard-level-values">
        <span><small>Hiện tại</small>{data.current}</span>
        <span><small>Dự đoán</small>{data.predicted}</span>
        <span><small>Mục tiêu</small>{data.target}</span>
      </div>
    </div>
  )
}
...
function StudentDashboard() {
  ...
  const visiblePracticeItems = testPractice.slice(practiceStartIndex, practiceStartIndex + practiceVisibleCount)
  ...
  <div className="hl-dashboard-goal-head">
    ...
    <div className="hl-dashboard-goal-coach">
      <div className="hl-dashboard-goal-bubble">
    ...
    <div className="hl-dashboard-locked">
          ...
          <section className="hl-dashboard-section">
            ...
            <article className="hl-dashboard-recent">
              ...
              <div>
                ...
                <p>{recentLesson.course} · {recentLesson.meta} <Star size={17} /> {recentLesson.score}</p>
              </div>
              <button type="button" onClick={showComingSoon}>Tiếp tục học <ArrowRight size={18} /></button>
            </article>
          </section>
            ...
            <div className="hl-dashboard-section-row">
              ...
              <div className="hl-dashboard-section-actions">
                <CarouselControls
                  ...
                  onNext={() => setCourseStartIndex((current) => Math.min(maxCourseStartIndex, current + 1))}
                />
                <button type="button" onClick={showComingSoon}>Xem tất cả</button>
              </div>
            </div>
            <div
              className="hl-dashboard-course-list"
              style={{ '--hl-dashboard-visible-cards': courseVisibleCount }}
            >
              {visibleCourses.map((course) => (
                <button key={course.id} type="button" className="hl-dashboard-mini-course" onClick={showComingSoon}>
                  ...
                  <div className="hl-dashboard-mini-course-body">
                    ...
                    <div>
                      ...
                      <strong><Trophy size={17} /> {course.score}</strong>
                    </div>
                  </div>
                </button>
              ))}
          ...
          <section className="hl-dashboard-profile-card">
            ...
            <div className="hl-dashboard-profile-box">
              ...
              <div className="hl-dashboard-summary-list">
                ...
                <p><Clock3 size={16} /> Tổng thời lượng <b>{dashboardSummary.studyTime}</b></p>
                <p><BookOpen size={16} /> Bài học đã hoàn thành <b>{dashboardSummary.completedLessons}</b></p>
                <p><ClipboardCheck size={16} /> Đề đã làm <b>{dashboardSummary.completedTests}</b></p>
                <p><Trophy size={16} /> Điểm cao nhất <b>{dashboardSummary.bestScore}</b></p>
                <p><Flame size={16} /> Chuỗi học tập <b>{dashboardSummary.currentStreak} ngày</b></p>
              </div>
            </div>
          </section>
        </aside>
      ...
      <section className="hl-dashboard-section hl-dashboard-practice-section">
        <div className="hl-dashboard-section-row">
          ...
          <div className="hl-dashboard-section-actions">
            <CarouselControls
              ...
              onNext={() => setPracticeStartIndex((current) => Math.min(maxPracticeStartIndex, current + 1))}
            />
            <button type="button" onClick={showComingSoon}>Xem tất cả</button>
          </div>
        </div>
        <div
          ...
          style={{ '--hl-dashboard-visible-cards': practiceVisibleCount }}
        >
          {visiblePracticeItems.map((item) => (
            ...
            >
              ...
              <h4>{item.title}</h4>
              <p>{item.questions} · {item.duration}</p>
