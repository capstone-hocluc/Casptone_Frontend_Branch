    ...
    <header className="hl-video-learn-header">
      <div>
        ...
        <span className="hl-video-logo">
          ...
          <b>HocLuc<span>.com</span></b>
        </span>
        <span />
        <strong>{course.title}</strong>
      </div>
      <img src={dashboardSummary.avatar || '/avatar-minhanh.jpg'} alt={dashboardSummary.studentName} />
    </header>
  )
}

function LessonNavBar({ context, previous, next, onOpenDrawer, onCourses, onNavigateActivity, onComplete }) {
  return (
    <div className="hl-video-lesson-nav">
      ...
      <div className="hl-video-breadcrumb">
        <button type="button" onClick={onCourses}>Khóa học của tôi</button>
      ...
      <div className="hl-video-nav-actions">
        <button type="button" disabled={!previous} onClick={() => previous && onNavigateActivity(previous.activity)}>
          <ChevronLeft size={16} />
          Bài trước
        </button>
        <button type="button" onClick={() => (next ? onNavigateActivity(next.activity) : onComplete())}>
...
function CourseLessonDrawer({ open, context, lessons, onClose, onAction, onNavigateActivity }) {
  ...
  return (
    <div className="hl-video-drawer-layer">
      <button type="button" className="hl-video-drawer-scrim" aria-label="Đóng danh sách bài học" onClick={onClose} />
      <aside className="hl-video-drawer">
        <div className="hl-video-drawer-head">
          <h2>Nội dung khóa học</h2>
          <button type="button" aria-label="Đóng danh sách bài học" onClick={onClose}><X size={18} /></button>
        </div>
              ...
              {items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span>{item.activity.type} · {item.activity.title}</span>
                  </button>
                )
              })}
            </section>
  ...
  return (
    <div ref={playerRef} className={`hl-video-player ${expanded ? 'is-expanded' : ''}`}>
      <div className="hl-video-player-label">
        <span>{activity.type === 'Buổi giải đề' ? 'Buổi giải đề' : 'Video'} · {activity.title}</span>
      </div>
      <button type="button" className="hl-video-play-big" aria-label={playing ? 'Tạm dừng video' : 'Phát video'} onClick={() => setPlaying((current) => !current)}>
        {playing ? <Pause size={42} /> : <Play size={46} />}
      </button>
      <div className="hl-video-controls">
        <button type="button" aria-label={playing ? 'Tạm dừng' : 'Phát'} onClick={() => setPlaying((current) => !current)}>
          {playing ? <Pause size={18} /> : <Play size={18} />}
        </button>
        <span>{playing ? '03:24' : '00:00'} / {formatDuration(activity.duration)}</span>
        <div><i style={{ width: playing ? '22%' : '0%' }} /></div>
        <Volume2 size={17} />
        <button type="button" aria-label="Phóng to video" onClick={toggleFullscreen}><Expand size={17} /></button>
      </div>
    </div>
  )
}
...
function TeacherAIButton({ state, onToggle }) {
  ...
  return (
    <button type="button" className="hl-video-ai-button" aria-label="Mở Teacher AI" onClick={onToggle}>
      <img src="/owl-mascot5.png" alt="" aria-hidden="true" />
    </button>
  )
}

function TeacherAIPanel({ state, lessonTitle, messages, input, onInput, onClose, onSend, onQuickAction }) {
  if (state === 'closed') return null
  ...
  return (
    <aside className="hl-video-ai-panel is-chat">
      <div className="hl-video-ai-chat-head">
        ...
        <button type="button" aria-label="Đóng Teacher AI" onClick={onClose}><X size={18} /></button>
      </div>
      <div className="hl-video-ai-messages">
        {messages.map((message) => (
          <p key={message.id} className={`is-${message.role}`}>{message.text}</p>
        ))}
      </div>
      <div className="hl-video-ai-quick">
        {quickActions.map((action) => (
          <button key={action} type="button" onClick={() => onQuickAction(action)}>{action}</button>
        ))}
      </div>
      <form className="hl-video-ai-input" onSubmit={onSend}>
        <input value={input} onChange={(event) => onInput(event.target.value)} placeholder="Hỏi trợ lý AI..." />
        <button type="submit" aria-label="Gửi câu hỏi"><Send size={16} /></button>
      </form>
    </aside>
  )
}
    ...
    return (
      <section className="hl-video-learning-page">
        <article className="hl-video-invalid">
          ...
          <button type="button" onClick={onBackCourse}><ArrowLeft size={16} />Quay lại khóa học</button>
        </article>
      </section>
    )
  }
